...
def process_instruction(line, current_file):
...
def execute_instructions_from_file(file_path, current_file):
    with open(file_path, 'r') as file:
        sections_added = set()
        values_found = False
        for line in file:
            ...
            values_found |= process_instruction(line.strip(), current_file)
...
def process_config_files():
    ...
    for current_file in run_files:
        if os.path.exists(current_file):
            # Effacer les valeurs spécifiées avant l'injection
            clear_target_values('config.ini', 'Player1 Device configuration', ['P1DeviceName'])

            values_found, sections_added = execute_instructions_from_file(current_file, current_file)

            if current_file == 'gun4ir.run':
    ...
    for section_to_add in sections_to_add:
        for i in range(2, 4):
            # Effacer les valeurs spécifiées avant l'injection
            clear_target_values('config.ini', f'Player{i} Device configuration', [f'P{i}DeviceName'])

            current_file = 'mayflash.run'
            execute_instructions_from_file(current_file, current_file)
