...
def clear_target_values(file_path, section):
...
def update_target_config(file_path, section, key, new_value):
    clear_target_values(file_path, section)
...
def process_instruction(line):
    ...
    if match:
        ...
        if section_source == 'clear' and field_source == 'clear':
            new_value = ''
        else:
        ...
        update_target_config(file_target, section_target, field_target, new_value)
        print(f"Valeur {new_value} écrite dans {section_target}[{field_target}] du fichier cible")
    else:
